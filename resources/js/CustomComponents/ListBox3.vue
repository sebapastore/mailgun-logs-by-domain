      ...
      >
        ...
        >
            <div v-show="open" class="absolute mt-1 w-full rounded-md bg-white shadow-lg" style="z-index: 9999;">
                <div class="relative pt-2 pb-1 pl-8 pr-4">
                    <input v-on:keyup="searchSelectOptions($event)" class="input-select border-gray-300 focus:border-indigo-300 focus:ring focus:ring-indigo-200 focus:ring-opacity-50 rounded-md shadow-sm leading-4 text-sm p-1" ref="search" type="text">
                </div>
            ...
            <span
                ...
                :class="selectedOption && selectedOption.provider_category_id === option.provider_category_id ? 'font-semibold' : 'font-normal'"
            >
              {{ option.name }}
            </span>
                        <span
                            v-show="selectedOption && selectedOption.provider_category_id === option.provider_category_id"
...
export default {
    ...
    mounted() {
        ...
        var option = this.options.find(option => option.provider_category_id === this.$attrs.modelValue)
    ...
    computed: {
        filters() {
            return this.options.filter(data => data.name.toLowerCase().includes(this.searchText))
        }
    },
    watch: {
        selectedOption: function () {
            this.$emit('input', this.selectedOption.provider_category_id)
        }
    }
}
</script>
